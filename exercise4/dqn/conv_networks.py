...
class CNN():
    ...
    def _build_model(self, state_dim, num_actions, hidden, lr):
        ...
        conv1 = tf.layers.conv2d(self.states_[:,:,:,0:1], 32, 7, activation=tf.nn.relu)
        pool1 = tf.layers.max_pooling2d(conv1, 2, 2)
        conv2 = tf.layers.conv2d(pool1, 48, 5, activation=tf.nn.relu)
        ...
        conv1_diff = tf.layers.conv2d(self.states_[:,:,:,1:2], 32, 7, activation=tf.nn.relu)
        pool1_diff = tf.layers.max_pooling2d(conv1_diff, 2, 2)
        conv2_diff = tf.layers.conv2d(pool1_diff, 48, 5, activation=tf.nn.relu)
        pool2_diff = tf.layers.max_pooling2d(conv2_diff, 2, 2)
        conv3_diff = tf.layers.conv2d(pool2_diff, 64, 3, activation=tf.nn.relu)
