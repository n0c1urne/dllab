...
class Model4(BaseModel):
    ...
    def define_model(self, x, y):
        ...
        state = cell.zero_state(64, dtype=tf.float32)

        for i in range(self.history_len):
            # first conv layer
            conv1 = tf.nn.conv2d(x[:,:,:,i:(i+1)], conv1_w, strides=[1, 1, 1, 1], padding='SAME') + conv1_b
            ...
            output, state = cell(fc2, state)

        # output layer - no dropout or relu here
        fc3_w = tf.Variable(tf.truncated_normal(shape=(32, fc3_size), mean=mu, stddev=sigma))
        fc3_b = tf.Variable(tf.zeros(fc3_size))
        fc3 = tf.matmul(output, fc3_w) + fc3_b
